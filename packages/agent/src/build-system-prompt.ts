...
export function buildSystemPrompt(nlLogic: string, endpoints: Endpoint[]): string {
	...
	const coreTools = [
		{ name: "scheduleNext", signature: "{ delayMinutes: number, reason: string }" },
	...
	const coreSection = coreTools
		.map((t) => `• **${t.name}(input: ${t.signature})** - pre-defined scheduling primitive`)
		.join("\n");

	// Format the user-defined webhook tools section
	const userSection = endpoints.map((ep) => `• **${ep.name}(...)** - calls webhook at ${ep.url}`).join("\n");
