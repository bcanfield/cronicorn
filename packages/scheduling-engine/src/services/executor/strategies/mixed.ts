...
export async function executeWithDependencies(
    ...
    const filtered = disabled ? plan.endpointsToCall.filter(e => !disabled.has(e.endpointId)) : plan.endpointsToCall;
    ...
    const executeAll = async (): Promise<void> => {
        const ready = filtered.filter(e => !completed.has(e.endpointId)).filter(areDepsMet);
        if (ready.length === 0) {
            if (completed.size < filtered.length) {
                const pending = filtered.filter(e => !completed.has(e.endpointId)).map(e => e.endpointId);
                if (pending.length > 0)
