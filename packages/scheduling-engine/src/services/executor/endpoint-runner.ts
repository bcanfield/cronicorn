...
export async function executeEndpointWithRetry(params: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    let aborted = false;
    try {
      log.info?.("endpoint_attempt", { endpointId: plannedEndpoint.endpointId, attempt, jobId });
      ...
      if (success) {
        ...
        log.info?.("endpoint_success", { endpointId: plannedEndpoint.endpointId, status: response.status, attempt, elapsedMs: Date.now() - attemptStart });
      ...
      log.error?.("endpoint_failure", { endpointId: plannedEndpoint.endpointId, status: response.status, attempt, classification: classification.category });
      events?.onEndpointProgress?.({ jobId, endpointId: plannedEndpoint.endpointId, status: "failed", attempt, error: `HTTP ${response.status}` });
      return { endpointId: plannedEndpoint.endpointId, success: false, statusCode: response.status, executionTimeMs: Date.now() - attemptStart, timestamp, responseContent: lastResponseContent, truncated, error: `HTTP ${response.status}`, attempts: attempt };
    }
    catch (err) {
      ...
      if (newState !== prevState)
        events?.onCircuitStateChange?.({ endpointId: plannedEndpoint.endpointId, from: prevState, to: newState, failures: circuitBreaker.getState(plannedEndpoint.endpointId).failures });
      log.error?.("endpoint_failure", { endpointId: plannedEndpoint.endpointId, attempt, error: msg, classification: classification.category });
      events?.onEndpointProgress?.({ jobId, endpointId: plannedEndpoint.endpointId, status: "failed", attempt, error: msg });
      return { endpointId: plannedEndpoint.endpointId, success: false, statusCode: lastStatus || 0, executionTimeMs: Date.now() - startTimeOverall, timestamp, error: msg, attempts: attempt };
    }
  }
  ...
  log.error?.("endpoint_exhausted", { endpointId: plannedEndpoint.endpointId, attempts: maxAttempts, error: lastError });
  return { endpointId: plannedEndpoint.endpointId, success: false, statusCode: lastStatus || 0, executionTimeMs: Date.now() - startTimeOverall, timestamp, error: lastError || "Unknown error", responseContent: lastResponseContent, truncated, attempts: maxAttempts };
}
